...
function buildTaskQuery(query, user) {
  ...
  if (user.role === "Employee" && user.employee)
    filter.assignedTo = user.employee;
  return filter;
}
...
const getTask = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    String(task.assignedTo._id) !== String(req.user.employee)
  ) {
...
const createTask = asyncHandler(async (req, res) => {
  ...
  const task = await Task.create({
    ...
    statusHistory: [
      { status: req.body.status || "Assigned", changedBy: req.user._id },
    ],
  });
...
const updateTask = asyncHandler(async (req, res) => {
  ...
  if (
    req.user.role === "Employee" &&
    String(task.assignedTo) !== String(req.user.employee)
  ) {
  ...
  Object.entries(req.body).forEach(([key, value]) => {
    if (req.user.role === "Employee" && !allowedForEmployee.includes(key))
      return;
    task[key] = value;
  });
